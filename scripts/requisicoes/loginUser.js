    ...
    response => {
      response.json().then(data => {
        ...
        if (response.status === 201) {
          ...
          // Somente para aguardar a entrada do usuário. E dar tempo da animação acontecer
          setTimeout(() => {
            location.href = 'tarefas.html'
          }, 1501)
        }
        if (response.status === 400) {
          Swal.fire('Opa!', 'Senha errada', 'error')
        }
        ...
        if (response.status === 500) {
          Swal.fire('Opa!', 'Temos um probleminha no servidor!', 'warning')
        }
      })
    }
  )
